...
/**@file   heur_greedylp.cpp
 * @ingroup DEFPLUGINS_HEUR
 * @brief  greedy LP primal heuristic
 * @author Erik Jansen
 */
...
#define HEUR_NAME            "greedylp"
#define HEUR_DESC            "greedy search for feasible primal solutions based on LP solution"
...
static
SCIP_DECL_HEUREXEC(heurExecGreedyLP)
{
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   SCIPdebugMsg(scip, "Running greedy lp search heuristic ...\n");
   ...
   SCIP_CALL( SCIPgetSolVals(scip, nullptr, (int) probdata->n, probdata->vars, primsol) );

   SCIPsortDownRealPtr(primsol, (void**) vars, (int) probdata->n);
   ...
   for (size_t i = 0; i < probdata->n; i++)
   {
      ...
      assert( SCIPisFeasEQ(scip, lpval, primsol[i]) );

      if ( i != 0 )
         assert( SCIPisFeasLE(scip, lpval, oldlpval) );
   }
#endif
   ...
   if ( stored )
   {
      *result = SCIP_FOUNDSOL;
      SCIPdebugMsg(scip, "Greedy LP heuristic found solution.\n");
   }
   else
   {
      *result = SCIP_DIDNOTFIND;
      SCIPdebugMsg(scip, "Greedy LP heuristic did not find new solution.\n");
   }
...
/** creates the greedy LP primal heuristic and includes it in SCIP */
SCIP_RETCODE BACSincludeHeurGreedyLP(
   ...
   )
{
   ...
   /* create greedylp primal heuristic data */
   SCIP_CALL(SCIPincludeHeurBasic(scip, &heur,
         HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecGreedyLP, heurdata));
   assert( heur != nullptr );
